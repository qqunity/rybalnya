...
func IsLiked(w http.ResponseWriter, r *http.Request) {
	...
	if utils.IsExpiredToken(r) {
		response := http_response.IsLikedResponseErr{
			Status: false,
			Error:  "Токен устарел!",
		}
		...
		if err != nil {
			...
			isLiked, exitCode := db_interaction.IsLiked(db_interaction.DBConnetor(true), uint64(postID), uint64(tokenMetadata.UserID),
				true)
			if exitCode == 0 {
				response := http_response.IsLikedResponseOk{
					Status:        true,
					Action:        "Информация о лайке успешно получена!",
					IsLiked:       isLiked,

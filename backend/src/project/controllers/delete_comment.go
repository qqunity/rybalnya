...
func DeleteComment(w http.ResponseWriter, r *http.Request) {
	...
	if utils.IsExpiredToken(r) {
		response := http_response.DeleteCommentResponseErr{
			Status: false,
			Error:  "Токен устарел!",
		}
		...
		id := r.FormValue("id")
		exitCode := db_interaction.DeleteComment(db_interaction.DBConnetor(true), id, true)
		if exitCode == 0 {
			response := http_response.DeleteCommentResponseOk{
				Status:        true,
				Action:        "Комментарий успешно удален!",

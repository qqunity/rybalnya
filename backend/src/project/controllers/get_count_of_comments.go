...
func GetCountOfCommentsByPostID(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	log.Print(fmt.Sprintf("REQUEST: Method |%s| \t Headers: |%s| \t Route: |%s|", r.Method, r.Header, r.RequestURI))
	if utils.IsExpiredToken(r) {
		response := http_response.GetCountOfCommentsResponseErr{
			Status: false,
			Error:  "Токен устарел!",
		}
		...
		if exitCode == 0 || exitCode == 1 {
			response := http_response.GetCountOfCommentsResponseOk{
				Status:        true,
				Action:        "Данные о комметариях успешно отправлены!",
				IsTokenExpire: utils.IsTokenExpire(r),
				CntComments: uint64(len(comments)),
			}
